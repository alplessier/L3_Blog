...
// OBTENTION DU TITRE ENTIER
$req2 = $pdo->prepare('SELECT title FROM articles WHERE id=?');
$req2->execute(array($id));
$reponse2 = $req2->fetch();

//OBTENTION DU CONTENU ENTIER
$req3 = $pdo->prepare('SELECT content FROM articles WHERE id=?');
            ...
            <section class="sectionArticle">
               ...
               <?php if(isset($_SESSION['username'])): ?>
                  <?php if($reponse['username'] == $_SESSION['username']): ?>
                     ...
                     <div id="outer">
                        <form action="../models/modifTitre.php" method="post">
                           <br>
                           <input type="text" maxlength="88" name="title" id="title" value="<?php echo $reponse2['title']; ?>"><br><br>
                           <textarea type="content" rows=6 cols=40 name="content" id="content" ><?php echo $reponse3['content']; ?></textarea><br><br>
                        ...
                        <form action="../models/suppArticle.php" method="post">
                           <input type="hidden" name="article_id" value=<?php echo $article -> id ?>/>
                           <input type="submit" name="Supprimer" value="Supprimer Article" class="boutton" id="bouttonSupp"><br><br>
                        </form>
                     </div>
                  <?php endif ?>
                <?php endif ?>
